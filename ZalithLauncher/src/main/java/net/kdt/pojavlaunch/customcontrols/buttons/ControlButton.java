...
public class ControlButton extends TextView implements ControlInterface {
    ...
    /* Pink/white animated glow */
    private static final int GLOW_COLOR_PINK = Color.parseColor("#FF6FB5");
    private static final int GLOW_COLOR_WHITE = Color.parseColor("#FFFFFF");
    private long mGlowStartTime = 0L;
    private final Runnable mGlowTicker = new Runnable() {
        @Override
        public void run() {
            invalidate();
            if (mIsToggled || (mProperties != null && !mProperties.isToggle && isActivated())) {
                postOnAnimationDelayed(this, 32L);
            }
        }
    };
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        boolean active = mIsToggled || (!mProperties.isToggle && isActivated());
        if (active) {
            if (mGlowStartTime == 0L) {
                mGlowStartTime = System.currentTimeMillis();
                postOnAnimationDelayed(mGlowTicker, 32L);
            }
            long elapsed = System.currentTimeMillis() - mGlowStartTime;
            float phase = (elapsed % 1400L) / 1400f;
            // Ping-pong 0..1..0 for a smooth breathing pink<->white sweep
            float t = phase < 0.5f ? phase * 2f : (1f - phase) * 2f;
            int blended = blendColor(GLOW_COLOR_PINK, GLOW_COLOR_WHITE, t);

            android.graphics.LinearGradient shader = new android.graphics.LinearGradient(
                    0, 0, getWidth(), getHeight(),
                    blended, GLOW_COLOR_WHITE,
                    android.graphics.Shader.TileMode.CLAMP);
            mRectPaint.setShader(shader);
            mRectPaint.setAlpha(mProperties.isToggle ? 150 : 90);
            canvas.drawRoundRect(0, 0, getWidth(), getHeight(), mComputedRadius, mComputedRadius, mRectPaint);
        } else {
            mGlowStartTime = 0L;
            removeCallbacks(mGlowTicker);
        }
    }

    private static int blendColor(int colorA, int colorB, float ratio) {
        float inverse = 1f - ratio;
        int a = (int) (Color.alpha(colorA) * inverse + Color.alpha(colorB) * ratio);
        int r = (int) (Color.red(colorA) * inverse + Color.red(colorB) * ratio);
        int g = (int) (Color.green(colorA) * inverse + Color.green(colorB) * ratio);
        int b = (int) (Color.blue(colorA) * inverse + Color.blue(colorB) * ratio);
